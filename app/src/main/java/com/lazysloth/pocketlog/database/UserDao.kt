...
interface UserDao {
    ...
    @Query("SELECT firebaseUid FROM users WHERE username = :username")
    suspend fun getIdByUsername(username: String): String?
    @Query("SELECT firebaseUid FROM users WHERE emailId = :emailId")
    suspend fun getIdByEmail(emailId: String): String?
